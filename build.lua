...
local function parse_markdown(content)
    ...
    if trimmed:match("^```") then
        if not in_code then
            ...
            table.insert(result, "<pre class='bg-dark text-white p-4 rounded-5 overflow-auto w-100'><code>" .. code_content .. "</code></pre>")
            code_block = {}
        end
        goto continue
    end
        ...
        if trimmed:match("^# ") then
            ...
            table.insert(result, "<h1>" .. text .. "</h1>")
            goto continue
        end
        ...
        if trimmed:match("^## ") then
            ...
            local text = trimmed:gsub("^## ", "")
            table.insert(result, "<h2>" .. text .. "</h2>")
            goto continue
        end
        ...
        if trimmed:match("^### ") then
            ...
            local text = trimmed:gsub("^### ", "")
            table.insert(result, "<h3>" .. text .. "</h3>")
            goto continue
        end
        ...
        if trimmed:match("^#### ") then
            ...
            local text = trimmed:gsub("^#### ", "")
            table.insert(result, "<h4>" .. text .. "</h4>")
            goto continue
        end
...
local function load_metadata()
    local content = read_file("metadata.yaml")
...
local function build_posts(metadata)
    ...
    for _, file in ipairs(files) do
        if file:match("%.md$") then
            ...
            if content then
                ...
                local tags = {}
                if fm.tags then
                    for tag in fm.tags:gmatch("[^,]+") do
                        local clean_tag = tag:gsub("^%s*(.-)%s*$", "%1")
                        if clean_tag ~= "" then table.insert(tags, clean_tag) end
                    end
                end
                
                local post = {
                    title = fm.title or slug,
                    date = fm.date or os.date("%Y-%m-%d"),
                    slug = slug,
                    excerpt = fm.excerpt or "",
                    description = fm.description or "",
                    tags = tags,
                    author = fm.author or "LUAX Team",
                    content = html,
                    url = "/" .. slug .. ".html",
                    image = fm.image or metadata.image,
                    layout = layout_name,
                }
                
                if post.excerpt == "" then
                    local text = html:gsub("<[^>]+>", "")
                    post.excerpt = text:sub(1, 150) .. "..."
                end
    ...
    for i, post in ipairs(posts) do
        post.index = i
        post.total = #posts
        if i > 1 then post.prev_post = posts[i-1] end
        if i < #posts then post.next_post = posts[i+1] end
    end
    return posts
end
...
local function format_date_for_rss(date_str)
    ...
    if year and month and day then
        ...
        return os.date("%a, %d %b %Y %H:%M:%S +0000", timestamp)
    end
...
for _, post in ipairs(posts) do
    ...
    local context = {
        ...
        json_ld_headline = page_title,
        json_ld_description = page_description,
        json_ld_url = page_url,
        json_ld_date = post.date,
        json_ld_author = post.author or "LUAX Team",
    }
...
for _, page in ipairs(pages) do
    ...
    local page_image = page.image  -- Sudah ada fallback
    ...
    local context = {
        ...
        json_ld_description = page_description,
        json_ld_url = page_url,
        json_ld_date = os.date("%Y-%m-%d"),
        json_ld_author = "LUAX Team",
    }
    context.metadata = metadata
...
if total_pages > 1 then
    ...
    if page_num > 1 then
        if page_num == 2 then
            pagination_html = pagination_html .. '<li class="page-item"><a class="page-link" href="/blog.html" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'
        else
            pagination_html = pagination_html .. '<li class="page-item"><a class="page-link" href="/blog/page/' .. (page_num - 1) .. '.html" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'
        end
    else
        pagination_html = pagination_html .. '<li class="page-item disabled"><span class="page-link" aria-hidden="true">&laquo;</span></li>'
    end
    
    -- Page numbers
    for i = 1, total_pages do
        if i == page_num then
            pagination_html = pagination_html .. '<li class="page-item active" aria-current="page"><span class="page-link">' .. i .. '</span></li>'
        else
            if i == 1 then
                pagination_html = pagination_html .. '<li class="page-item"><a class="page-link" href="/blog.html">' .. i .. '</a></li>'
            else
                pagination_html = pagination_html .. '<li class="page-item"><a class="page-link" href="/blog/page/' .. i .. '.html">' .. i .. '</a></li>'
            end
        end
    end
    ...
    if page_num < total_pages then
        pagination_html = pagination_html .. '<li class="page-item"><a class="page-link" href="/blog/page/' .. (page_num + 1) .. '.html" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>'
    else
        pagination_html = pagination_html .. '<li class="page-item disabled"><span class="page-link" aria-hidden="true">&raquo;</span></li>'
    end
    ...
    local context = {
        title = page_num == 1 and "Blog" or "Blog - Page " .. page_num,
        description = "All blog posts",
        posts = page_posts,
        pagination = pagination_html,
        current_url = page_num == 1 and "/blog.html" or "/blog/page/" .. page_num .. ".html",
        base_url = page_num == 1 and "" or "..",
        og_type = "website",
        og_title = page_num == 1 and "Blog" or "Blog - Page " .. page_num,
        og_description = "All blog posts",
        og_image = metadata.image or "",
        og_url = page_num == 1 and "/blog.html" or "/blog/page/" .. page_num .. ".html",
        json_ld_type = "WebPage",
        json_ld_headline = page_num == 1 and "Blog" or "Blog - Page " .. page_num,
        json_ld_description = "All blog posts",
        json_ld_url = page_num == 1 and "/blog.html" or "/blog/page/" .. page_num .. ".html",
        json_ld_date = os.date("%Y-%m-%d"),
        json_ld_author = "LUAX Team",
    }
    context.metadata = metadata
    ...
    if page_num == 1 then 
        write_file("dist/blog.html", html)
    else 
        write_file("dist/blog/page/" .. page_num .. ".html", html) 
    end
end
...
if posts and #posts > 0 then
    ...
    for _, post in ipairs(posts) do
        if post.tags and #post.tags > 0 then
            for _, tag in ipairs(post.tags) do
                -- Clean tag: lower case dan replace spasi dengan -
                local clean_tag = tag:lower():gsub(" ", "-")
                if not all_tags[clean_tag] then 
                    all_tags[clean_tag] = {
                        name = tag,  -- nama asli dengan spasi
                        posts = {}
                    }
                end
                table.insert(all_tags[clean_tag].posts, post)
            end
        end
    end
    ...
    if #tag_names > 0 then
        local tags_context = {
            title = "Tags - " .. (metadata.title or "LUAX SSG"),
            description = "All tags- " .. (metadata.title or "LUAX SSG"),
            tags = tag_names,
            base_url = "",
            current_url = "/tags",
            og_type = "website",
            og_title = "Tags - " .. (metadata.title or "LUAX SSG"),
            og_description = "All tags",
            og_image = metadata.image or "",
            og_url = "/tags",
            json_ld_type = "WebPage",
            json_ld_headline = "Tags",
            json_ld_description = "All tags",
            json_ld_url = "/tags",
            json_ld_date = os.date("%Y-%m-%d"),
            json_ld_author = "LUAX Team",
        }
        ...
        -- BUILD EACH TAG PAGE
        for clean_tag, data in pairs(all_tags) do
            local tag_posts = data.posts
            table.sort(tag_posts, function(a, b) return a.date > b.date end)
            
            local post_count = #tag_posts
            
            local tag_context = {
                title = data.name .. " - " .. (metadata.title or "LUAX SSG"),
                description = "Posts tagged with " .. data.name .. " (" .. post_count .. " posts)",
                tag = data.name,
                tag_clean = clean_tag,
                posts = tag_posts,
                post_count = post_count,
                debug_posts = #tag_posts,
                base_url = "..",
                current_url = "/tags/" .. clean_tag,
                og_type = "website",
                og_title = data.name .. " - " .. (metadata.title or "LUAX SSG"),
                og_description = "Posts tagged with " .. data.name .. " (" .. post_count .. " posts)",
                og_image = metadata.image or "",
                og_url = "/tags/" .. clean_tag,
                json_ld_type = "WebPage",
                json_ld_headline = data.name,
                json_ld_description = "Posts tagged with " .. data.name .. " (" .. post_count .. " posts)",
                json_ld_url = "/tags/" .. clean_tag,
                json_ld_date = os.date("%Y-%m-%d"),
                json_ld_author = "LUAX Team",
            }
...
local index_context = {
    ...
    base_url = "",
    og_type = "website",
    og_title = metadata.title or "LUAX SSG",
    og_description = metadata.description or "",
    og_image = metadata.image or "",
    og_url = site_url,
    json_ld_type = "WebSite",
    json_ld_headline = metadata.title or "LUAX SSG",
    json_ld_description = metadata.description or "",
    json_ld_url = site_url,
    json_ld_date = os.date("%Y-%m-%d"),
    json_ld_author = "LUAX Team",
}
